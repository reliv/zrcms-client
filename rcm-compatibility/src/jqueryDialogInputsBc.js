...
export default function (jQuery) {
    ...
    (
        function (jQuery) {
            ...
            var selectFieldFunction = function (description, choices, value, allowCustomValues) {
                ...
                var name = jQuery.fn.generateUUID();

                var p = jQuery('<p class="dialogElement" data-dialogElementName="' + name + '"></p>');
                var selected;
                ...
                var select = jQuery('<select name="' + name + '"><select>');
                ...
                if (allowCustomValues) {
                    ...
                    inputBox = jQuery('<input' + displayNone + ' size="80" value="' + customValue + '">');
                }
            ...
            var methods = {
                image: function (description, src) {
                    ...
                    var p = jQuery('<p class="dialogElement imageInput" data-dialogElementName="' + name + '" style="overflow-y:hidden"></p>');
                    p.append(
                    ...
                    var urlBox = jQuery('<input style="width:370px;margin-right:10px" name="' + name + '" value="' + src + '">');
                    p.append(urlBox);
                    p.append(
                    ...
                    if (!inputImageEventsDelegated) {
                        ...
                        jQuery('body').on(
                            ...
                            function () {
                                showFileBrowserForInputBox(
                                    jQuery(this).parent().children('input')
                                    , 'images'
                                );
                            }
                        );
                        ...
                        jQuery('body').on(
                            'change', '.imageInput input', function () {
                                jQuery(this).parent().children('img').attr('src', jQuery(this).val());
                            }
                        );
                    }
                ...
                textWithAjaxValidator: function (description, value, urlToValidator, disallowSpaces, successCallback) {
                    ...
                    var validatorId = jQuery.fn.generateUUID();
                    var name = jQuery.fn.generateUUID();
                    ...
                    jQuery('body').on(
                        'keyup', "#" + name, function () {
                            var validationContainer = jQuery("#" + validatorId);
                ...
                password: function (description, validationDescription, value) {
                    ...
                    //Give it a random name so labels and multi-dialogs work
                    var validationName = jQuery.fn.generateUUID();

                    var validationP = jQuery(
                    ...
                    var divId = jQuery.fn.generateUUID();

                    var div = jQuery("<div></div>").append(p).append(validationP);

                    jQuery('body').on(
                        'keyup', "#" + validationName, function () {
                            var passwordField = jQuery("#" + name);
                            var validationField = jQuery("#" + validationName);

                            var password = jQuery(passwordField).val();
                            var validationPassword = jQuery(validationField).val();

                            if (password !== validationPassword) {
                                jQuery(passwordField).addClass('RcmErrorInputHightlight');
                                jQuery(passwordField).removeClass('RcmOkInputHightlight');
                                jQuery(validationField).addClass('RcmErrorInputHightlight');
                                jQuery(validationField).removeClass('RcmOkInputHightlight');
                            } else {
                                jQuery(passwordField).removeClass('RcmErrorInputHightlight');
                                jQuery(passwordField).addClass('RcmOkInputHightlight');
                                jQuery(validationField).removeClass('RcmErrorInputHightlight');
                                jQuery(validationField).addClass('RcmOkInputHightlight');
                            }
                        }
                    );
                ...
                date: function (description, value) {
                    ...
                    var input = jQuery('<input name="' + name + '" value="' + value + '">');
                    p.append(input);
                    input.datepicker();
                ...
                richEdit: function (description, value) {
                    ...
                    var id = jQuery.fn.generateUUID();
                    var div = jQuery('<div id="' + id + '" data-rcm-html-edit data-html-editor-attached-toolbar="true" data-html-editor-type="simpleText">' + value + '</div>');
                    var p = jQuery(
                ...
                validateInput: function (inputField, resultContainer, ajaxPath, disallowSpaces, successCallback) {
                    ...
                    if (disallowSpaces) {
                        ...
                        inputValue = jQuery(inputField).val().toLowerCase().replace(
                            /\s/g,
                            '-'
                        ).replace(/[^A-Za-z0-9\-\_]/g, "");
                        jQuery(inputField).val(inputValue);
                    } else {
                        inputValue = jQuery(inputField).val();
                    }
                ...
                inputFieldError: function (inputField, resultContainer) {
                    jQuery(resultContainer).removeClass('ui-icon-check');
                    jQuery(resultContainer).addClass('ui-icon-alert').addClass('ui-icon');
                    jQuery(inputField).addClass('RcmErrorInputHightlight');
                    jQuery(inputField).removeClass('RcmOkInputHightlight');

                },

                inputFieldFatalError: function (inputField, resultContainer) {
                    jQuery(resultContainer).html('<p style="color: #FF0000;">Error!</p>');
                    jQuery(inputField).addClass('RcmErrorInputHightlight');
                    jQuery(inputField).removeClass('RcmOkInputHightlight');
                },

                inputFieldOk: function (inputField, resultContainer) {
                    jQuery(resultContainer).removeClass('ui-icon-alert');
                    jQuery(resultContainer).addClass('ui-icon-check').addClass('ui-icon');
                    jQuery(inputField).removeClass('RcmErrorInputHightlight');
                    jQuery(inputField).addClass('RcmOkInputHightlight');
                },
                ...
                getDialogElementVal: function () {
                    ...
                    if (newVal == null) {
                        ...
                        if (typeof(name) != 'undefined') {
                            ...
                            if (jQuery(dialogElement).hasClass('imageInput')) {
                                jQuery(dialogElement).children('img').attr('src', newVal);
                            }
             ...
             * @type {Function}
             */
            var originalProp = jQuery.fn.prop;
             ...
             * @constructor
             */
            jQuery.fn.prop = function (propertyName, value) {
             ...
             * @return {Object}
             */
            jQuery.fn.dialogIn = function (inputType, label, option1, option2, option3) {
             ...
             * @param inputs
             */
            jQuery.fn.appendMulti = function (inputs) {
    ...
    jQuery.dialogIn = function (inputType, description, value, options, depricatedUsedToBeOption3) {
        return jQuery.fn.dialogIn.apply(this, arguments);
    };

    jQuery.generateUUID = function () {
        return jQuery.fn.generateUUID();
    };
    /**
     * </jquery-dialog-inputs>
     */
